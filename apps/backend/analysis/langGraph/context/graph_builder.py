...
class GraphBuilder:
    """Compatibility surface around repository graph refresh and traversal."""

    def __init__(self, manager: RepositoryGraphManager | None = None) -> None:
        self._manager = manager or RepositoryGraphManager()

    def build(self, *, repo_id: str) -> int:
        return self._manager.refresh_graph(repo_id=repo_id)

    def neighbors(self, *, repo_id: str, path: str, depth: int = 2, limit: int = 32) -> list[str]:
